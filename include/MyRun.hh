...
public:
	...
	const G4int* GetPMTResponse(int pidx = 0) const
	{
		return fPMTResponse[pidx];
	}
	...
	const G4int *GetGammaSourceSpec() const
	{
		return spec_theroy[0];
	}
	const G4int *GetNeutronSourceSpec() const
	{
		return spec_theroy[1];
	}
	
	const G4int *GetGammaSourceMultiplicity() const
	{
		return multi_theroy[0];
	}
	const G4int *GetNeutronSourceMultiplicity() const
	{
		return multi_theroy[1];
	}
	const G4int *GetJointSourceMultiplicity() const
	{
		return multi_theroy[2];
	}
	...
	G4int* GetRossiCX(int id) const
	{
		return rossi_cx[id];
	}
	G4int* DetectedSpectrum() const
	{
		return n_spec;
	}

	G4int** GetAngularContour() const
        {
                return angular_contour;
        }
	G4int** GetAngularPlot() const
        {
                return angular_plot;
        }

	G4int** GetAngularCCXontour() const
        {
                return angular_contourcx;
        }
	G4int** GetAngularCXPlot() const
        {
                return angular_plotcx;
        }
...
private:
	...
	void ProcessCoincidence(std::map<double, entry> storage, std::vector<int>& angular, std::vector<int>& angularcx, int &multi, int &multi_cx, int* rossi, int* rossicxs) ;
