...
enum ptype { PMT = 0, optical = 1, proton = 2, neutron = 3, electron = 4, Cerenkov = 5, holder = 6 };
enum rtype { compt = 0, hadElastic = 1, eIoni = 2, hIoni = 3, alpha = 4 };
class RecodedParticle{
	public:
	RecodedParticle(int pid, std::string nm, G4double eng, G4double tm, G4ThreeVector dir){
...
class RecodedEvent 
{
  public:
	...
	void delete_class()
	{
		...
		for (int i = 0; i < 7; i++)
		{
			if (intervaltimeEnergy[i] != NULL)
				free(intervaltimeEnergy[i]);
			if (intervaltimeCounter[i] != NULL)
	...
	bool IsValid()
	{
		ParticleType();
		//return true;
		if (name == "neutron" && reacEnergy[idx][1]/keV < 1*depoEnergy[idx][0]/keV) return false; //4 1.5 
		
		if (name == "neutron" && ptr_particledef->EntryEnergy/keV < reacEnergy[idx][1]/keV) return false;
		
		//if (name == "neutron" && reacEnergy[idx][1]/keV < 3.5*(cutoff)) return false; //4 1.5
	...
	int ParticleType ()
	{
		...
		{
			if (reacEnergy[i][0] > engp) {engp = reacEnergy[i][0]; idp = i;}
			if (reacEnergy[i][1] > engn) {engn = reacEnergy[i][1]; idn = i;}
			
		}
		if (engp > engn*2) {ptr_particledef = &particledef.at(idp);  idx = idp;}
	...
	G4double GetTriggerTime(G4double cutoff)
	{
		...
		// if (name == "gamma" && reacEnergy[idx][0]/keV < (cutoff)) return triggerTime;
		...
		for (; it != triggermap[idx].end(); it++)
		 ...
		 //G4cout << "GetTriggerTime cutoff depoEnergy " << cutoff << " " << depoEnergy[idx][0]/keV << " " << triggerTime  << G4endl;
		 return triggerTime;
	...
	void Print()
	{
		...
		{
			G4cout << " ----------------- " << G4endl;
			G4cout << "    Particle No " << particledef.at(i).particleid << (idx == i ? " Accepted " : " ")  
					<<  particledef.at(i).name  << G4endl;
			G4cout << "    Particle I Energy " << G4BestUnit(particledef.at(i).IncidentEnergy, "Energy") << G4endl;
			G4cout << "    Particle E Energy " << G4BestUnit(particledef.at(i).EntryEnergy, "Energy") << G4endl;			
			G4cout << "    firstDepo[i][0] " << G4BestUnit(firstDepo[i][0], "Time") << G4endl;
			G4cout << "    firstDepo[i][1] " << G4BestUnit(firstDepo[i][1], "Time") << G4endl;
			G4cout << "    depoEnergy[i][0] " << G4BestUnit(depoEnergy[i][0], "Energy") << G4endl;
			G4cout << "    depoEnergy[i][1] " << G4BestUnit(depoEnergy[i][1], "Energy") << G4endl;
			G4cout << "    firstInteraction[i][0] " << G4BestUnit(firstInteraction[i][0], "Time") << G4endl;
			G4cout << "    firstInteraction[i][1] " << G4BestUnit(firstInteraction[i][1], "Time") << G4endl;
			G4cout << "    reacEnergy[i][0] " << G4BestUnit(reacEnergy[i][0], "Energy") << G4endl;
			G4cout << "    reacEnergy[i][1] " << G4BestUnit(reacEnergy[i][1], "Energy") << G4endl;		
			G4cout << " ----------------- " << G4endl;
		}
		G4cout << " \\\\\\\\\\\\\\ " << G4endl;
