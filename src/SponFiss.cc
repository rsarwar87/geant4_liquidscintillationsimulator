...
void SponFiss::GeneratePrimaryVertex(G4Event* anEvent, G4double time,
  ...
  if (3 == fe->getCorrelationOption()) {
    ...
    if (err_len > 1) {
      ...
      ed << "Call to new fissionEvent("
         ...
         << "eng=0."
         << ", "
         << "0) failed with error message from FREYA: " << G4endl
         << error_message;
  ...
  if (verbosityLevel > 1) {
    G4cout << " nPrompt: " << nPrompt << "   gPrompt: " << gPrompt << G4endl;
  }
  ...
  for (int n1 = 0; n1 < nPrompt; n1++) {
    double u1 = fe->getNeutronDircosu(n1), v1 = fe->getNeutronDircosv(n1),
           w1 = fe->getNeutronDircosw(n1);
    for (int n2 = n1 + 1; n2 < nPrompt; n2++) {
      double u2 = fe->getNeutronDircosu(n2), v2 = fe->getNeutronDircosv(n2),
             w2 = fe->getNeutronDircosw(n2);
    ...
    for (G4int i = nPrompt - 1; i > -1; i--) {
      ...
      for (int n2 = i + 1; n2 < nPrompt; n2++)
      {
              double u2 = fe->getNeutronDircosu(n2), v2 =
                              fe->getNeutronDircosv(n2), w2 =
      fe->getNeutronDircosw( n2); double scalar_product = u1 * u2 + v1 * v2 + w1
      * w2;
