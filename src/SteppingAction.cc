...
void SteppingAction::Reset()
{
	...
	for (int i = 0; i < 2; i++)
		if (rParticle[i].size() > 0)
		{
			...
			{
				if (rParticle[i].at(j) != NULL)
					delete rParticle[i].at(j);
	...
	crEvent = NULL;
	crParticle = NULL;
	_cnnt = 0; _cnnt2 = -1; _eng_l = 1; _tm_l = 1000000;
	return;
}
//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo......
...
void SteppingAction::StackParticle(const G4Step* step, const G4StepPoint * point)
{
	...
	if ((_eng_l !=  step->GetPreStepPoint()->GetKineticEnergy()) 
			&& ( _cnnt2 != track->GetTrackID()))
		{
			...
			crParticle = new RecodedParticle(_cnnt, ParticleName, _eng_l, 
							track->GetGlobalTime(),point->GetMomentumDirection () );
			// if (ParticleName != "neutron") crParticle->Pout(_cnnt*1000+track->GetParentID() );
			trackInfo->fID = _cnnt;
...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
	...
	{
		//if (step->GetPreStepPoint()->GetKineticEnergy()/keV < 300) track->SetTrackStatus(fStopAndKill);
	...
	else if (trackInfo->GetTrackingStatus() > 0) 
	{
		...
		if (( track->GetDynamicParticle()->GetParticleDefinition()->GetParticleName() == "gamma" ) && _eng_l != track->GetParentID())
    		{
			StackParticle(step, step->GetPreStepPoint());
...
//	if (trackInfo->GetTrackingStatus() > 0) 
	{
		...
		{
			//G4cout << " Particle not found " << track->GetKineticEnergy()/keV << " " << trackInfo->fParentID << " " << _cnnt << G4endl;
			return;
		}
		else if (trackInfo->fID != crParticle->particleid)
		{
			...
			{
				//G4cout << " Particle not found " << track->GetKineticEnergy()/keV << " " << trackInfo->fParentID  << G4endl;
				return;
			}
		}
	}
	...
	// collect information on the detectors in which the simulation is taking place
	...
	if ((crEvent == NULL)  || (crEvent->detectorID != idx_det))
	{
		...
		if (found != -1)
			...
			crEvent = new RecodedEvent(idx_det, crParticle, step->GetTrack()->GetGlobalTime()) ;
	...
	// for optical photons, detect the boundary absorption os the particles and count them
			...
			== "opticalphoton")
	{
		...
		if (step->GetPostStepPoint()->GetStepStatus() == fGeomBoundary)
		{
			...
			if (boundaryStatus == Detection)
			{
				...
				crEvent->recordProduction(0, step->GetPreStepPoint()->GetTotalEnergy()*425.4885 ,
	...
	if (ParticleName == "neutron" || ParticleName == "gamma")
	{
				...
				== "compt")
		{
			...
			crEvent->recordReaction(0, deng, step->GetPostStepPoint()->GetGlobalTime(), 
				...
				== "hadElastic")
		{
			...
			crEvent->recordReaction(1, deng, step->GetPostStepPoint()->GetGlobalTime(), 
	...
	if (ParticleName == "proton")
	{
		...
		if (step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName()
				== "hIoni")
		{
			//if (step->GetPreStepPoint()->GetKineticEnergy() == step->GetPostStepPoint()->GetKineticEnergy() ) return;
			G4double deng = step->GetPreStepPoint()->GetKineticEnergy()
							- step->GetPostStepPoint()->GetKineticEnergy();
			...
			if (secondaries->at(i)->GetParentID() > 0)
			{
						...
						== G4OpticalPhoton::OpticalPhotonDefinition())
				{
					if (secondaries->at(i)->GetCreatorProcess()->GetProcessName()
							== "Scintillation")
					{
						crEvent->recordProduction(1,step->GetPostStepPoint()->GetGlobalTime(), secondaries->at(i)->GetKineticEnergy());
					}
					if (secondaries->at(i)->GetCreatorProcess()->GetProcessName()
							== "Cerenkov")
					{
						crEvent->recordProduction(5, secondaries->at(i)->GetKineticEnergy());
					}
				}
				else if (secondaries->at(i)->GetDynamicParticle()->GetParticleDefinition()->GetParticleName()
						== "neutron" && ParticleName != "neutron"
						&& track->GetDynamicParticle()->GetParticleDefinition()->GetParticleName()
								!= "proton")
				{
					//G4cout << secondaries->at(i)->GetVolume()->GetName()<< " " << ParticleName << " " << secondaries->at(i)->GetCreatorProcess()->GetProcessName()  << G4endl;
					//G4cout << secondaries->at(i)->GetDynamicParticle()->GetParticleDefinition()->GetParticleName()<< "\n";
								...
								!= "e-")
				{
					crEvent->recordProduction(4, secondaries->at(i)->GetKineticEnergy());
                                                ...
                                                == "gamma" )
                                {
                                  /*      G4cout << G4RunManager::GetRunManager()->GetCurrentEvent()->GetEventID() << " Gamma detected " << 
						  secondaries->at(i)->GetKineticEnergy()/MeV << " " << secondaries->at(i)->GetParentID() << G4endl;
					G4cout << "Parent " <<  ParticleName << "\t" << step->GetPostStepPoint()->GetTotalEnergy()/keV 
						<< "MeV \t"<< step->GetPreStepPoint()->GetKineticEnergy()/keV  << " keV \t" <<
						  step->GetPostStepPoint()->GetKineticEnergy() << " keV \t" 
						  << step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName() << " keV \t"<< G4endl;
					//secondaries->at(i)->SetTrackStatus(fStopAndKill);
					G4cout << step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessName() << " " << 
						step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessType() << " " << step->GetPostStepPoint()->GetProcessDefinedStep()->GetProcessSubType() << G4endl;
					*/
				}
