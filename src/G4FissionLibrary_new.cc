...
// void G4FissionLibrary_new::Init (G4double A, G4double Z, G4String & dirName,
// G4String &)
void G4FissionLibrary_new::Init(G4double A, G4double Z, G4int M,
  ...
  G4ParticleHPDataUsed aFile = theNames.GetName(
      static_cast<G4int>(A), static_cast<G4int>(Z), M, dirName, tString, dbool);
  ...
  while (theData >> infoType)  // Loop checking, 22.03.2015. T. Koi
  {
    ...
    switch (infoType) {
      ...
      case 5:
        ...
        break;
      default:
        G4cout << "G4FissionLibrary_new::Init: unknown data type" << dataType
               << G4endl;
        throw G4HadronicException(
            __FILE__, __LINE__,
            "G4FissionLibrary_new::Init: unknown data type");
        break;
    }
  }
...
G4HadFinalState* G4FissionLibrary_new::ApplyYourself(
  ...
  G4ThreeVector neuVelo =
      (1. / incidentParticle->GetDefinition()->GetPDGMass()) *
      theNeutron.GetMomentum();
  theTarget = aNucleus.GetBiasedThermalNucleus(
      targetMass, neuVelo, theTrack.GetMaterial()->GetTemperature());
  ...
  // no energy conservation on an event-to-event basis. we rely on the data to
  // be ok. @@ also for mean, we rely on the consistency of the data. @@
  ...
  for (G4int i = 0; i < nPrompt; i++) {
    ...
    //    G4cout <<"G4FissionLibrary_new::ApplyYourself: energy of prompt
    //    neutron " << i << " = " << it->GetKineticEnergy()<<G4endl;
  }

  // Build gammas, lorentz transform them, and add them to dynamic particle
  // vector
  for (G4int i = 0; i < gPrompt; i++) {
    ...
    //    G4cout <<"G4FissionLibrary_new::ApplyYourself: energy of prompt photon
    //    " << i << " = " << it->GetKineticEnergy()<<G4endl;
    // theResult.AddSecondary(it);     // geant4.10.00
    theResult.Get()->AddSecondary(it);  // geant4.10.01
    delete thePhoton;
  }
  ...
  /*
           ...
           G4ParticleDefinition* theKindOfFragment =
     theTableOfIons->GetIon(Zs[i],As[i],0.0); G4ReactionProduct *
     theFissionFragment = new G4ReactionProduct(theKindOfFragment);
  ...
  //  G4cout <<"G4FissionLibrary_new::ApplyYourself: Number of induced prompt
  //  neutron = "<<nPrompt<<G4endl;                         // geant4.10.00
  //  G4cout <<"G4FissionLibrary_new::ApplyYourself: Number of secondaries =
  //  "<<theResult.Get()->GetNumberOfSecondaries()<< G4endl; // geant4.10.01
  //  G4cout <<"G4FissionLibrary_new::ApplyYourself: Number of induced prompt
  //  photons = "<<gPrompt<<G4endl;
  ...
  //   G4cout << "G4FissionLibrary_new::local energy deposit" <<
  //   eDepByFragments<<G4endl;
...
void G4FissionLibrary_new::SampleMult(const G4HadProjectile& theTrack,
                                      G4int* nPrompt, G4int* gPrompt,
                                      G4double eKinetic) {
  G4double promptNeutronMulti = 0;
  promptNeutronMulti =
      theFinalStateNeutrons.GetPrompt(eKinetic);  // prompt nubar from Geant
  G4double delayedNeutronMulti = 0;
  delayedNeutronMulti =
      theFinalStateNeutrons.GetDelayed(eKinetic);  // delayed nubar from Geant
  ...
  if (delayedNeutronMulti == 0 && promptNeutronMulti == 0) {
    ...
    // but there is perhaps data for the total neutron multiplicity, in which
    // case we use it for prompt neutron emission
    if (fe != 0) delete fe;
  ...
  if (3 == fe->getCorrelationOption()) {
    ...
    if (err_len > 1) {
      ...
      ed << "eKinetic=" << eKinetic << ", "
         << "1) failed with error message from FREYA: " << G4endl
         << error_message;
      delete[] error_message;
      G4Exception("G4FissionLibrary_new::SampleMult", "freya001",
                  FatalException, ed);
    }
    delete[] error_message;
  }
  ...
  *nPrompt = fe->getNeutronNu();
  if (*nPrompt == -1)
    *nPrompt = 0;  // the fission library libFission.a has no data for neutrons
  *gPrompt = fe->getPhotonNu();
  if (*gPrompt == -1)
    *gPrompt = 0;  // the fission library libFission.a has no data for gammas
}
#endif
