		...
		G4VUserPrimaryGeneratorAction(), fParticleGun(0)
{
	...
	{
		currentIsotope = new SponFissIsotope(
				static_cast<G4int>(isotopeList->operator[](i)));
		...
		if (i == 0)
			fissionSource = new MultipleSource(currentIsotope,
					intensityList->operator[](i));
		else
			fissionSource->AddaSource(currentIsotope,
					intensityList->operator[](i));
