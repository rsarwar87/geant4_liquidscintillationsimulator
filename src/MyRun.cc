...
Run::Run(DetectorConstruction *det)
  ...
  fout.open(filename /*+ PrimaryGeneratorAction::name*/, std::ios::out);
  /*	fout << "\n\n" << std::setw(8) << "EventId" << "  " << std::setw(14)
                          << "   GlobalTime" << "  " << std::setw(5) << "DetID"
     << "  "
                          << std::setw(5) << "ParID" << "  " << std::setw(10)
                          << " PrimaryEnrg" << "  " << std::setw(10) <<
     "PrimaryDepo" << "  "
                          << std::setw(6) << "PriCnt" << "  " << "  " <<
     std::setw(10)
                          << "ProtnEnrg" << "  " << std::setw(10) <<
     "ProtonDepo" << "  "
                          << std::setw(6) << "ProCnt" << "  " << std::setw(10)
     << "ElectEnrg"
                          << "  " << std::setw(10) << "ElectDepo" << "  " <<
     std::setw(6)
...
void Run::Merge(const G4Run *run) {
  ...
  for (itn = localRun->fParticleDataMap.begin();
    ...
    if (fParticleDataMap.find(name) == fParticleDataMap.end()) {
      fParticleDataMap[name] = ParticleData(localData.fCount, localData.fEmean,
                                            localData.fEmin, localData.fEmax);
    } else {
  ...
  for (int k = 0; k < 500; k++) n_spec[k] += localRun->n_spec[k];

  for (int i = 0; i < DETECTOR_COUNT; i++)
    for (int k = 0; k < 500; k++) _time[i][k] += localRun->_time[i][k];
...
void Run::EndOfRun() {
  ...
  G4cout << "\n   track length        E>1*eV= "
         << G4BestUnit(meanTrackLen1, "Length")
...
void Run::RecordEvent(const G4Event *evt) {
  ...
  if (eContainer->size() > 0) {
    for (int i = 0; i < eContainer->size(); i++) {
      ...
      if ((tmp->depoEnergy[idpn][PMT] == 0) && (tmp->reacEnergy[idpn][PMT] > 0))
        continue;
      int fPartType = (tmp->name == "neutron" ? 1 : 0);
      if ((tmp->depoEnergy[idpn][PMT] == 0) &&
          (tmp->depoEnergy[idpn][PMT] > tmp->reacEnergy[idpn][fPartType]))
        continue;
      if (!tmp->IsValid()) continue;

      if (fPartType == 1)
        dNeutrons.push_back(tmp);
      else
        dPhotons.push_back(tmp);
      dJoint.push_back(tmp);

      if ((tmp->particledef.at(0).IncidentEnergy / keV) < 9555)
        fIncidentEnergy[fPartType][(int)(tmp->ptr_particledef->IncidentEnergy /
                                         keV / 2)]++;
      if ((tmp->particledef.at(0).EntryEnergy / keV) < 9555)
        fEntryEnergy[fPartType]
                    [(int)(tmp->ptr_particledef->EntryEnergy / keV / 2)]++;

      if (tmp->reacEnergy[idpn][fPartType] / keV < 9555)
        fParticleDeposit[fPartType]
                        [(int)(tmp->reacEnergy[idpn][fPartType] / keV / 2)]++;

      if (tmp->reacEnergy[idpn][2] / keV < 9555)
        fElectronDeposited[fPartType]
                          [(int)(tmp->reacEnergy[idpn][2] / keV / 2)]++;

      if (tmp->depoEnergy[idpn][electron] / keV < 9555)
        fElectronProduced[fPartType]
                         [(int)(tmp->depoEnergy[idpn][electron] / keV / 2)]++;

      if (tmp->depoEnergy[idpn][proton] / keV > 0 &&
          tmp->depoEnergy[idpn][proton] / keV < 9555)
        fProtonProduced[fPartType]
                       [(int)(tmp->depoEnergy[idpn][proton] / keV / 2)]++;

      if (tmp->reacEnergy[idpn][hIoni] / keV > 0 &&
          tmp->reacEnergy[idpn][hIoni] / keV < 9555)
        fProtonDeposited[fPartType]
                        [(int)(tmp->reacEnergy[idpn][hIoni] / keV / 2)]++;

      if (tmp->depoEnergy[idpn][optical] / keV > 0 &&
          tmp->depoEnergy[idpn][optical] / keV < 9555)
        fOphotonProduced[fPartType]
                        [(int)(tmp->depoEnergy[idpn][optical] / keV / 2)]++;
      if (tmp->depoEnergy[idpn][Cerenkov] / keV > 0 &&
          tmp->depoEnergy[idpn][Cerenkov] / keV < 9555)
        fCerenkovProduced[fPartType]
                         [(int)(tmp->depoEnergy[idpn][Cerenkov] / keV / 2)]++;
  ...
  for (int i = 0; i < dNeutrons.size(); i++) {
    ...
    // G4cout << i <<" y " <<  t <<  " " << dNeutrons.at(i)->depoEnergy[PMT]/keV
    // <<G4endl;
    if (t < 0) continue;
    entry tmp_;
    ...
    if (t < 500)
      if (i > 0 || PrimaryGeneratorAction::mono) n_spec[(int)t]++;
    dNeutrons.at(i)->printTimeSeries(fout);
  }

  // if (dNeutrons.size() > 0) G4cout << dNeutrons.size() << " " <<
  // stNeutron.size() << G4endl;
  ...
  for (int i = 0; i < dPhotons.size(); i++) {
    ...
    stPhoton[t] = tmp_;
    stJoint[t] = tmp_;
    dPhotons.at(i)->printTimeSeries(fout);
  }
  ...
  if ((stNeutron.size() > 0))
    if (stNeutron.size() == 1) {
      ...
      ProcessCoincidence(stNeutron, angular, angularcx, multi_n, multi_nxc,
                         rossi_[1], rossi_cx[1]);
  ...
  if ((stPhoton.size() > 0))
    if (stPhoton.size() == 1) {
      ...
      ProcessCoincidence(stPhoton, ang, an, multi_g, multi_gxc, rossi_[0],
                         rossi_cx[0]);
  an.clear();
  ang.clear();
  if ((stJoint.size() > 0))
    if (stJoint.size() == 1) {
      ...
      ProcessCoincidence(stJoint, ang, an, multi_j, multi_jxc, rossi_[2],
                         rossi_cx[2]);
...
void Run::ProcessCoincidence(std::map<double, entry> storage,
                             std::vector<int> &angular,
                             std::vector<int> &angularcx, int &multi,
                             int &multi_cx, int *rossi, int *rossicx) {
  ...
  for (; it != storage.end(); it++) {
    ...
    if (tim == -1) {
      ...
      if (!cx_map[en.pid]) {
        ...
        angularcx.push_back(en.didx);
        rossicx[deltaT]++;
        // G4cout << "ProcessCoincidence cx angular" << angularcx.size() <<
        // G4endl; G4cout << "ProcessCoincidence cs rossi" << (int) (it->first -
        // tim) << G4endl;
      } else
        _time[en.didx][deltaT]++;
      cx_map[en.pid] = true;
      // G4cout << "ProcessCoincidence angular" << angular.size() << G4endl;
      // G4cout << "ProcessCoincidence rossi" << (int) (it->first - tim) <<
      // G4endl;
    } else if (deltaT < 500) {
