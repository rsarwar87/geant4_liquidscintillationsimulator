...
NeutronHPMessenger::NeutronHPMessenger(NeutronHPphysics* phys) :
		G4UImessenger(), fNeutronPhysics(phys), fPhysDir(0), fThermalCmd(0)
{
	fPhysDir = new G4UIdirectory("/testhadr/phys/");
	fPhysDir->SetGuidance("physics list commands");

	fThermalCmd = new G4UIcmdWithABool("/testhadr/phys/thermalScattering",
			this);
...
void NeutronHPMessenger::SetNewValue(G4UIcommand* command, G4String newValue)
{
	...
	{
		fNeutronPhysics->SetThermalPhysics(
				fThermalCmd->GetNewBoolValue(newValue));
	}
}
