...
void RunAction::multiplicity_spec(const G4Run *aRun) {
  std::ofstream fout;
  fout.open(
      filename + "_spec_" + std::to_string(PrimaryGeneratorAction::energy),
  ...
  for (int pid = 0; pid < 2; pid++) {
    ...
    for (uint i = 0; i < 4500; i++)
      fout << std::setw(sp) << i * 2 + 1 << "\t" << std::setw(sp)
           << fIncidentEnergy[i] << "\t" << std::setw(sp) << fEntryEnergy[i]
           << "\t" << std::setw(sp) << fParticleDeposit[i] << "\t"
           << std::setw(sp) << fProtonDeposited[i] << "\t" << std::setw(sp)
...
void RunAction::multiplicity_th() {
          ...
          std::ios::out | std::ios::trunc);
  {
    ...
    for (int i = 0; i < 32; i++) fd << "\t" << g_th[i];
    fd << "\nTheoretical Multiplicity: Neutron\n";
    for (int i = 0; i < 32; i++) fd << "\t" << n_th[i];
    fd << "\nTheoretical Multiplicity: Joint\n";
    for (int i = 0; i < 64; i++) fd << "\t" << j_th[i];
  }
  {
    ...
    for (int i = 0; i < 32; i++) fd << "\t" << g_ac[i];
    fd << "\nXT Corrected Multiplicity: Neutron\n";
    for (int i = 0; i < 32; i++) fd << "\t" << n_ac[i];
    fd << "\nXT Corrected Multiplicity: Joint\n";
    for (int i = 0; i < 64; i++) fd << "\t" << j_ac[i];
  }

  G4int *n_spec = fRun->DetectedSpectrum();

  fd << "\n\n\nSpec\n";
  for (int k = 0; k < 500; k++) fd << "\t" << n_spec[k];
  fd << "\n";

  fd << "\n\n\nRossi\n";
  for (int i = 0; i < 3; i++) {
    G4int *Rossi = fRun->GetRossi(i);
    for (int k = 0; k < 500; k++) fd << "\t" << Rossi[k];
    fd << "\n";
  }
  fd << "\n\n\nRossi CX\n";
  for (int i = 0; i < 3; i++) {
    G4int *Rossi_cx = fRun->GetRossiCX(i);
    for (int k = 0; k < 500; k++) fd << "\t" << Rossi_cx[k];
    fd << "\n";
  }
  ...
  fd << "\n totals ==>";
  for (int i = 0; i < 16; i++) fd << hist[0][i] << "\t";

  fd << "\n singlets ==>";
  for (int i = 0; i < 16; i++) fd << hist[1][i] << "\t";

  fd << "\n couplets ==>";
  for (int i = 0; i < 16; i++) fd << hist[2][i] << "\t";

  fd << "\n triplets ==>";
  for (int i = 0; i < 16; i++) fd << hist[3][i] << "\t";

  fd << "\n quarts ==>";
  for (int i = 0; i < 16; i++) fd << hist[4][i] << "\t";

  fd << "\n pentlets ==>";
  for (int i = 0; i < 16; i++) fd << hist[5][i] << "\t";

  fd << "\n\n octopus ==>\n";
  for (int i = 0; i < 16; i++) {
    for (int j = 0; j < 16; j++) fd << hist3[i][j] << " ";
    fd << "\n";
  }
  ...
  fd << "\n\n angu_th ==>\n";
  for (int i = 0; i < 100; i++) {
    for (int j = 0; j < 10; j++) fd << DetectorConstruction::hist[j][i] << " ";
    fd << "\n";
  }
  fd.close();
...
void RunAction::EndOfRunAction(const G4Run *aRun) {
  ...
  if (IsMaster()) {
    fTimer->Stop();
    G4cout << "Global result with " << fRun->gEventNumber << " " << *fTimer
           << G4endl;
  } else {
