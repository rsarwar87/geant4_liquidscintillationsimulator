...
void DetectorConstruction::DefineMaterials()
{
	...
	{ 2.034 * eV, 2.068 * eV, 2.103 * eV, 2.139 * eV, 2.177 * eV, 2.216 * eV,
			2.256 * eV, 2.298 * eV, 2.341 * eV, 2.386 * eV, 2.433 * eV, 2.481
					* eV, 2.532 * eV, 2.585 * eV, 2.640 * eV, 2.697 * eV, 2.757
					* eV, 2.820 * eV, 2.885 * eV, 2.954 * eV, 3.026 * eV, 3.102
					* eV, 3.181 * eV, 3.265 * eV, 3.353 * eV, 3.446 * eV, 3.545
					* eV, 3.649 * eV, 3.760 * eV, 3.877 * eV, 4.002 * eV, 4.136
					* eV };

	const G4int nEntries = sizeof(photonEnergy) / sizeof(G4double);

	G4double refractiveIndex1[] =
	{ 1.3435, 1.344, 1.3445, 1.345, 1.3455, 1.346, 1.3465, 1.347, 1.3475, 1.348,
			1.3485, 1.3492, 1.35, 1.3505, 1.351, 1.3518, 1.3522, 1.3530, 1.3535,
			1.354, 1.3545, 1.355, 1.3555, 1.356, 1.3568, 1.3572, 1.358, 1.3585,
			1.359, 1.3595, 1.36, 1.3608 };

	assert(sizeof(refractiveIndex1) == sizeof(photonEnergy));

	G4double absorption[] =
	{ 3.448 * m, 4.082 * m, 6.329 * m, 9.174 * m, 12.346 * m, 13.889 * m, 15.152
			* m, 17.241 * m, 18.868 * m, 20.000 * m, 26.316 * m, 35.714 * m,
			45.455 * m, 47.619 * m, 52.632 * m, 52.632 * m, 55.556 * m, 52.632
					* m, 52.632 * m, 47.619 * m, 45.455 * m, 41.667 * m, 37.037
					* m, 33.333 * m, 30.000 * m, 28.500 * m, 27.000 * m, 24.500
					* m, 22.000 * m, 19.500 * m, 17.500 * m, 14.500 * m };
...
//***Materials
	...
	//Vacuum
	fVacuum = new G4Material("Vacuum", z = 1., a = 1.01 * g / mole, density =
	...
	{ 1.56962 * eV, 1.58974 * eV, 1.61039 * eV, 1.63157 * eV, 1.65333 * eV,
			1.67567 * eV, 1.69863 * eV, 1.72222 * eV, 1.74647 * eV, 1.77142
					* eV, 1.7971 * eV, 1.82352 * eV, 1.85074 * eV, 1.87878 * eV,
			1.90769 * eV, 1.93749 * eV, 1.96825 * eV, 1.99999 * eV, 2.03278
					* eV, 2.06666 * eV, 2.10169 * eV, 2.13793 * eV, 2.17543
					* eV, 2.21428 * eV, 2.25454 * eV, 2.29629 * eV, 2.33962
					* eV, 2.38461 * eV, 2.43137 * eV, 2.47999 * eV, 2.53061
					* eV, 2.58333 * eV, 2.63829 * eV, 2.69565 * eV, 2.75555
					* eV, 2.81817 * eV, 2.88371 * eV, 2.95237 * eV, 3.02438
					* eV, 3.09999 * eV, 3.17948 * eV, 3.26315 * eV, 3.35134
					* eV, 3.44444 * eV, 3.54285 * eV, 3.64705 * eV, 3.75757
					* eV, 3.87499 * eV, 3.99999 * eV, 4.13332 * eV, 4.27585
					* eV, 4.42856 * eV, 4.59258 * eV, 4.76922 * eV, 4.95999
					* eV, 5.16665 * eV, 5.39129 * eV, 5.63635 * eV, 5.90475
					* eV, 6.19998 * eV };
	const G4int numentries_water = sizeof(energy_water) / sizeof(G4double);
	G4double mie_water[] =
	{ 167024.4 * m, 158726.7 * m, 150742 * m, 143062.5 * m, 135680.2 * m,
			128587.4 * m, 121776.3 * m, 115239.5 * m, 108969.5 * m, 102958.8
					* m, 97200.35 * m, 91686.86 * m, 86411.33 * m, 81366.79 * m,
			76546.42 * m, 71943.46 * m, 67551.29 * m, 63363.36 * m, 59373.25
					* m, 55574.61 * m, 51961.24 * m, 48527.00 * m, 45265.87 * m,
			42171.94 * m, 39239.39 * m, 36462.50 * m, 33835.68 * m, 31353.41
					* m, 29010.30 * m, 26801.03 * m, 24720.42 * m, 22763.36 * m,
			20924.88 * m, 19200.07 * m, 17584.16 * m, 16072.45 * m, 14660.38
					* m, 13343.46 * m, 12117.33 * m, 10977.70 * m, 9920.416 * m,
			8941.407 * m, 8036.711 * m, 7202.470 * m, 6434.927 * m, 5730.429
					* m, 5085.425 * m, 4496.467 * m, 3960.210 * m, 3473.413 * m,
			3032.937 * m, 2635.746 * m, 2278.907 * m, 1959.588 * m, 1675.064
					* m, 1422.710 * m, 1200.004 * m, 1004.528 * m, 833.9666 * m,
			686.1063 * m };

	assert(sizeof(mie_water) == sizeof(energy_water));

	// gforward, gbackward, forward backward ratio
	G4double mie_water_const[3] =
	{ 0.99, 0.99, 0.8 };
	// gforward, gbackward, forward backward ratio
	G4MaterialPropertiesTable* matH2O = new G4MaterialPropertiesTable();

	matH2O->AddProperty("RINDEX", photonEnergy, refractiveIndex1, nEntries)->SetSpline(
			true);
	matH2O->AddProperty("ABSLENGTH", photonEnergy, absorption, nEntries)->SetSpline(
			true);
	matH2O->AddProperty("MIEHG", energy_water, mie_water, numentries_water)->SetSpline(
			true);
	...
	G4double RefractiveIndexPethylene2[] =
	{ 1.42, 1.42, 1.42, 1.42 };
	...
	G4double RefractiveIndexPethylene1[] =
	{ 1.49, 1.49, 1.49, 1.49 };
	...
	Pethylene1Properties->AddProperty("ABSLENGTH", wls_Energy, AbsFiber,
			wlsnum);
	...
	{ 3.2204 * eV, 3.0996 * eV, 2.9876 * eV, 2.9173 * eV, 2.8700 * eV, 2.8114
			* eV, 2.3616 * eV };
	const G4int scintillatornum = sizeof(scintillator_Energy)
			/ sizeof(G4double);

	G4double SCY_Energy[201] =
	{ 0.01000*MeV, 0.1000*MeV, 0.2000*MeV, 0.3000*MeV, 0.4000*MeV, 0.5000*MeV, 0.6000*MeV, 0.7000*MeV, 
		0.8000*MeV, 0.9000*MeV, 1*MeV, 1.100*MeV, 1.200*MeV, 1.300*MeV, 1.400*MeV, 1.500*MeV, 
		1.600*MeV, 1.700*MeV, 1.800*MeV, 1.900*MeV, 2*MeV, 2.100*MeV, 2.200*MeV, 2.300*MeV, 2.400*MeV,
			fExpHall_z / 2);

	G4LogicalVolume* expHall_log = new G4LogicalVolume(expHall_box, fAir,
			"World", 0, 0, 0);

	G4VPhysicalVolume* expHall_phys = new G4PVPlacement(0, G4ThreeVector(),
			expHall_log, "World", 0, false, 0, fCheckOverlaps);
	...
	if (!lancs) DefineORNL(expHall_log, 15);
	else DefineLANCS(expHall_log);
...
void DetectorConstruction::DefineLANCS(G4LogicalVolume* expHall_log)
{
	DefineRoom(expHall_log);
	 ...
	 //Tank
	DefineTank(expHall_log);
	new G4PVPlacement(0, G4ThreeVector(-140. * cm, 7. * cm, -110.5 * cm), //rotation,position
	...
	"Detector",             //its name
			...
			4,                 //copy number
			fCheckOverlaps);       // checking overlaps*/
	new G4PVPlacement(0, G4ThreeVector(-138. * cm, 20. * cm, -110.5 * cm), //rotation,position
	...
	"Detector",             //its name
			...
			4,                 //copy number
			fCheckOverlaps);       // checking overlaps*/
	new G4PVPlacement(0, G4ThreeVector(-136. * cm, 33. * cm, -110.5 * cm), //rotation,position
	...
	new G4PVPlacement(0, G4ThreeVector(-130. * cm, 45. * cm, -110.5 * cm), //rotation,position
	...
	new G4PVPlacement(0, G4ThreeVector(-125. * cm, 55. * cm, -110.5 * cm), //rotation,position
	...
	new G4PVPlacement(0, G4ThreeVector(-120. * cm, 67. * cm, -110.5 * cm), //rotation,position
	...
	"Detector",             //its name
			...
			4,                 //copy number
			fCheckOverlaps);       // checking overlaps*/
	new G4PVPlacement(0, G4ThreeVector(-117. * cm, -80. * cm, -110.5 * cm), //rotation,position
	...
	new G4PVPlacement(
			G4Transform3D(rotm,
					G4ThreeVector(-120. * cm, 30. * cm, -80.5 * cm)), //rotation,position
...
void DetectorConstruction::DefineORNL(G4LogicalVolume* logicWorld, int nd)
{
	...
	G4Box* solidCeiling = new G4Box("CeilingG", 0.5 * fExpHall_x,
			0.5 * fExpHall_y, 0.5 * Ceilingz); //its size
	...
	G4LogicalVolume* logic_tablebase = new G4LogicalVolume(solid_tablebase,
			fAl,
			solid_tablebase->GetName());
	G4Box* solid_TableLeg = new G4Box("T_leg", 0.5 * l_tleg, 0.5 * b_tleg,
			0.5 * h_tleg); //its size
	G4LogicalVolume* logicTableLeg = new G4LogicalVolume(solid_TableLeg, fAl,
			solid_TableLeg->GetName());
	...
	G4Box* solid_support = new G4Box("Support", 0.5 * l_sup, 0.5 * b_sup,
			0.5 * h_sup); //its size
	G4Box* solid_support2 = new G4Box("Support", 0.5 * (l_sup - 1),
			0.5 * (b_sup - 1), 0.5 * h_sup); //its size
	G4LogicalVolume* logic_support = new G4LogicalVolume(solid_support, fAl,
			solid_support->GetName());
	G4LogicalVolume* logic_support2 = new G4LogicalVolume(solid_support2, fAir,
			solid_support->GetName());
	...
	G4LogicalVolume* logicRing = new G4LogicalVolume(solidRing,      //its solid
			fAir,         //its material
			"Ring");             //its name
	G4LogicalVolume* logicRingSup = new G4LogicalVolume(solidRingSupport,      //its solid
			fAir,         //its material
	...
	new G4PVPlacement(0,                       //no rotation
			G4ThreeVector(0 * cm, 0 * cm, table_center + l_sup + cryst_dX / 2), //at (0,0,0)
	...
	G4Tubs* pb_tube1 = new G4Tubs("pb_shield", pb_rad2, pb_rad1, pb_dz / 2, 0.,
			twopi);
	G4LogicalVolume* volPb = new G4LogicalVolume(pb_tube1, fPb,
			pb_tube1->GetName());
	new G4PVPlacement(0,                       //no rotation
	...
	G4Tubs* logic_can = new G4Tubs("logic_can", 0, src_rad1, can1_dz / 2, 0.,
			twopi);
	G4Tubs* logic_src = new G4Tubs("SRC", 0, src_rad2, (can2_dz) / 2, 0.,
			twopi);

	G4LogicalVolume* volSrccan = new G4LogicalVolume(logic_can, fAl,
			logic_can->GetName());
	G4LogicalVolume* volSrc = new G4LogicalVolume(logic_src, fCf_src,
			logic_src->GetName());
	...
	if (ring_W1 == 0* cm)
	{
		...
		G4Tubs* logic_tube1 = new G4Tubs("logic_tube1", 0, tube1_rad1, tube1_dz / 2,
				0., twopi);
		G4Tubs* logic_tube2 = new G4Tubs("logic_tube2", 0, tube2_rad1,
				(tube2_dz) / 2, 0., twopi);
		G4Tubs* logic_tube1i = new G4Tubs("logic_tube2i", 0, tube1_rad2,
				tube1_dz2 / 2, 0., twopi);
		G4Tubs* logic_tube2i = new G4Tubs("logic_tube2i", 0, tube2_rad2,
				(tube2_dz2) / 2, 0., twopi);
		G4LogicalVolume* volTube1 = new G4LogicalVolume(logic_tube1, fAl,
				logic_tube1->GetName());
		G4LogicalVolume* volTube2 = new G4LogicalVolume(logic_tube2, fAl,
				logic_tube2->GetName());
		G4LogicalVolume* volTube1i = new G4LogicalVolume(logic_tube1i, fAir,
				logic_tube1i->GetName());
		G4LogicalVolume* volTube2i = new G4LogicalVolume(logic_tube2i, fAir,
				logic_tube2i->GetName());
		new G4PVPlacement(0,                       //no rotation
		...
		G4Box* solid_support = new G4Box("Support", 0.5 * l_sup, 0.5 * b_sup,
				0.5 * h_sup); //its size
		G4Box* solid_support2 = new G4Box("Support", 0.5 * (l_sup),
				0.5 * (b_sup - 1), 0.5 * (h_sup - 1)); //its size
		G4LogicalVolume* logic_support = new G4LogicalVolume(solid_support, fAl,
				solid_support->GetName());
		G4LogicalVolume* logic_support2 = new G4LogicalVolume(solid_support2, fAir,
				solid_support->GetName());
		...
		new G4PVPlacement(0,                       //no rotation
				...
				fCheckOverlaps);         // checking overlaps
		new G4PVPlacement(0,                       //no rotation
				G4ThreeVector(0 * cm, 0 * cm, table_center + h_sup + tube1_dz / 2), //at (0,0,0)
		...
		new G4PVPlacement(
				...
				G4ThreeVector(0 * cm, 0 * cm,
						table_center + h_sup + tube1_dz + tube2_dz / 2), //at (0,0,0)
		...
		new G4PVPlacement(
				0,                       //no rotation
				G4ThreeVector(0 * cm, 0 * cm,
						table_center + h_sup + tube1_dz + tube2_dz + can1_dz / 2), //at (0,0,0)
				...
				0,                       //copy number
				fCheckOverlaps);         // checking overlaps
		source_position = G4ThreeVector(0 * cm, 0 * cm,
						table_center + h_sup + tube1_dz + tube2_dz + can1_dz / 2);
	}
	else
	{
		G4double water_dz = (-63*cm - table_center) ;
		G4Tubs* logic_can = new G4Tubs("water_can", 0, ring_W1, water_dz, 0.,
			twopi);
		G4LogicalVolume* volWater = new G4LogicalVolume(logic_can, fwater,
				logic_can->GetName());
		...
		G4LogicalVolume *fPhotocath_log)
{
  ...
  //**Photocathode surface properties
	...
	{ .25, .25 }; //Enables 'detection' of photons
...
G4LogicalVolume* DetectorConstruction::DefineEJ309(bool upright, int copyNo)
{
	...
	G4double scin_frame_dx = 10 * cm, scin_frame_dy = 10 * cm, scin_frame_dz =
			40 * cm;
	...
	// Solids
	G4Box* solidDetectorFrame = new G4Box("DetectorFrame",
			scin_frame_dx / 2,
			scin_frame_dy / 2, scin_frame_dz / 2); //detectorBlock
	G4Box* solidScintillatorChamber = new G4Box("ScintillatorChamber",
			scin_endcap_dxy / 2, scin_endcap_dxy / 2, scin_endcap_dz / 2); //AlEndCap

	G4Tubs* SolidGlassFrame = new G4Tubs("GlassFrame", glass_frame_rad2,
			glass_frame_rad1, glass_frame_dz / 2, 0., twopi); //detectorBlock
	//G4IntersectionSolid IntersFrames("IntersFrames", SolidGlassFrame, solidDetectorFrame);

	G4Tubs* SolidglassBlock1 = new G4Tubs("GlassBlock1", 0, glass_rad1,
			glass_dz1 / 2, 0., twopi);
	G4Tubs* SolidglassBlock2 = new G4Tubs("GlassBlock2", 0, glass_rad1,
			glass_dz / 2, 0., twopi);
	G4Cons* solidPMTCone = new G4Cons("PMTCone", 0, tube1_rad1,	//   //G4double  pRmin1, G4double  pRmax1,
			0, tube2_rad1, (1.75), 0, 2 * twopi / 3);
	G4Tubs* solidPMTTopTube = new G4Tubs("PMTTopTube", 0, tube3_rad1,
			(tube2_dz + tube3_dz) / 2, 0., twopi);
	G4Tubs* solidPMTBottomTube = new G4Tubs("PMTBottomTube", 0, tube1_rad1,
			(tube1_dz) / 2, 0., twopi);
	...
	//G4LogicalVolume* logicDetECap1 = new G4LogicalVolume(solidDetECap, fAl, "DetEndCap");
	...
	G4LogicalVolume* logicGlassFrame = new G4LogicalVolume(SolidGlassFrame, fAl,
			SolidGlassFrame->GetName());            //its name
	//G4LogicalVolume* logicGlassFrame = new G4LogicalVolume(&IntersFrames, fAl, "GlassFrame");            //its name
	G4LogicalVolume* logicDetGlass1 = new G4LogicalVolume(SolidglassBlock1,
			fPMMA, SolidglassBlock1->GetName());          //its name
	G4LogicalVolume* logicDetGlass2 = new G4LogicalVolume(SolidglassBlock2,
			fPMMA, SolidglassBlock2->GetName());
	G4LogicalVolume* logicPMTCone = new G4LogicalVolume(solidPMTCone, fAl,
			solidPMTCone->GetName());
	G4LogicalVolume* logicPMTSTube = new G4LogicalVolume(solidPMTTopTube, fAl,
			solidPMTTopTube->GetName());            //its name
	G4LogicalVolume* logicPMTBTube = new G4LogicalVolume(solidPMTBottomTube,
			fAl, solidPMTBottomTube->GetName());

	if (upright)
	{
		solidScintillationChamber = new G4Box("Scintillator",
				scin_room_chamber_dxy / 2, (scin_room_chamber_dxy) / 2,
				scin_room_dz * 0.6 / 2); //detectorchamber
		logicScintillationChamber = new G4LogicalVolume(
				solidScintillationChamber, fscintillator,
				solidScintillationChamber->GetName());
		solidEmptyChamber = new G4Box("EmptyChamber", scin_room_chamber_dxy / 2,
				(scin_room_chamber_dxy) / 2, scin_room_dz * 0.4 / 2); //detectorchamberEmpty
		...
		logicScintillationChamber, solidScintillationChamber->GetName(), //its name
		...
		logicEmptyChamber, solidEmptyChamber->GetName(),              //its name
				logicScintillatorChamber,              //its mother  volume
				false, copyNo, fCheckOverlaps);
	}
	else
	{
		solidEmptyChamber = new G4Box("EmptyChamber", scin_room_chamber_dxy / 2,
				(scin_room_chamber_dxy * 0.4) / 2, scin_room_dz / 2); //detectorchamberEmpty
		logicEmptyChamber = new G4LogicalVolume(solidEmptyChamber, fAir,
				solidEmptyChamber->GetName());
		solidScintillationChamber = new G4Box("Scintillator",
				scin_room_chamber_dxy / 2, (scin_room_chamber_dxy * 0.6) / 2,
		...
		logicScintillationChamber, logicScintillationChamber->GetName(), //its name
				logicScintillatorChamber,              //its mother  volume
				false, copyNo, fCheckOverlaps);
		new G4PVPlacement(0, G4ThreeVector(0, 2.7 * cm, 0),         //at (0,0,0)
		logicEmptyChamber, logicEmptyChamber->GetName(),              //its name
				logicScintillatorChamber,              //its mother  volume
				false, copyNo, fCheckOverlaps);
	}
	...
	logicScintillatorChamber, logicScintillatorChamber->GetName(),    //its name
	...
	logicPMTSTube, logicPMTSTube->GetName(),              //its name
			...
			false, copyNo, fCheckOverlaps);
	/*new G4PVPlacement(0,
			G4ThreeVector(-h_sup / 2 + (scin_frame_dx + h_sup) / 2, 0, -5 * cm), //at (0,0,0)
			logic_support, logic_support->GetName(),              //its name
			logicDetectorFrame,              //its mother  volume
			false, copyNo, fCheckOverlaps);*/
...
void DetectorConstruction::DefineChamber(G4LogicalVolume* logicWorld)
{
	...
	G4LogicalVolume* logicEmpty = new G4LogicalVolume(geom_empty,    //its solid
	...
	G4Box* geom_empty_frame = new G4Box("BuldgeFrame", 0.5 * (X),
			0.5 * fBubble_y, 0.5 * Z); //its size
	G4LogicalVolume* logicEmptyFrame = new G4LogicalVolume(geom_empty_frame,
			fAl, geom_empty_frame->GetName());
	...
	G4Box* geam_inner_linning = new G4Box("InnerLinning", 0.5 * (X),
			0.5 * fBubble_y, 0.5 * Z); //its size
	G4LogicalVolume* logicInnerLinning = new G4LogicalVolume(geam_inner_linning,
			fCd, geam_inner_linning->GetName());
	new G4PVPlacement(0,                       //no rotation
			G4ThreeVector(0, 0, 0.0),         //at (0,0,0)
	...
	G4Box* geam_polly1 = new G4Box("Polly1", 0.5 * (X), 0.5 * fBubble_y,
			0.5 * Z); //its size
	G4LogicalVolume* logicPolly1 = new G4LogicalVolume(geam_polly1, fAir,
			geam_polly1->GetName());
	new G4PVPlacement(0,                       //no rotation
			G4ThreeVector(0, 0, 0.0),         //at (0,0,0)
	...
	G4Box* geam_polly2 = new G4Box("Polly2", 0.5 * (X), 0.5 * fBubble_y,
			0.5 * Z); //its size
	G4LogicalVolume* logicPolly2 = new G4LogicalVolume(geam_polly2, fAir,
			geam_polly2->GetName());
	new G4PVPlacement(0,                       //no rotation
			G4ThreeVector(0, 0, 0.0),         //at (0,0,0)
	...
	G4Box* geam_Outer = new G4Box("OuterLinning", 0.5 * (X), 0.5 * fBubble_y,
			0.5 * Z); //its size
	G4LogicalVolume* logicOuterLinning = new G4LogicalVolume(geam_Outer, fCd,
			geam_Outer->GetName());
	new G4PVPlacement(0,                       //no rotation
			G4ThreeVector(0, 0, 0.0),         //at (0,0,0)
	...
	G4Box* geam_polly4 = new G4Box("Polly4", 0.5 * (X), 0.5 * fBubble_y,
			0.5 * Z); //its size
	G4LogicalVolume* logicPolly4 = new G4LogicalVolume(geam_polly4, fAir,
			geam_polly4->GetName());
	new G4PVPlacement(0,                       //no rotation
			G4ThreeVector(0, 0, 0.0),         //at (0,0,0)
	...
	X += outter_steel * 2;
	Z += outter_steel * 2;
	G4Box* geam_steel = new G4Box("Steel", 0.5 * (X), 0.5 * fTank_y, 0.5 * Z); //its size
	G4LogicalVolume* logicSteel = new G4LogicalVolume(geam_steel, fSteel,
			geam_steel->GetName());
	new G4PVPlacement(0,                       //no rotation
			G4ThreeVector(0, 0, 0.0),         //at (0,0,0)
	...
	G4double Y = 5.1 * cm;
	X = Z = 90.16 * cm;
	G4Box* geam_plate = new G4Box("Plate", 0.5 * (X), 0.5 * Y, 0.5 * Z); //its size
	G4Box* geam_plateCd = new G4Box("CdPlate", 0.5 * (X), 0.5 * 0.05, 0.5 * Z); //its size
	G4LogicalVolume* logicPlateCd = new G4LogicalVolume(geam_plateCd, fCd,
			geam_plate->GetName());
	G4LogicalVolume* logicPlateN = new G4LogicalVolume(geam_plate, fPethylene1,
			geam_plate->GetName());
	new G4PVPlacement(0,                       //no rotation
	...
	new G4PVPlacement(0,                       //no rotation
			...
			0,                       //copy number
			fCheckOverlaps);         // checking overlaps
	G4LogicalVolume* logicPlateF = new G4LogicalVolume(geam_plate, fPethylene1,
			geam_plate->GetName());
	new G4PVPlacement(0,                       //no rotation
...
G4LogicalVolume* DetectorConstruction::DefineHE3(bool upright, int copyNo)
{
	...
	G4Box* solidDetectorFrame = new G4Box("DetectorFrame", 5 * cm / 2,
			fBubble_y / 2, 5 * cm / 2); //detectorBlock
	G4LogicalVolume* logicDetectorFrame = new G4LogicalVolume(
	...
	G4Tubs* SolidAlFrame = new G4Tubs("AlFrame", 0, 5 * cm / 2, fBubble_y / 2,
			0., twopi);
	G4LogicalVolume* logicAlFrame = new G4LogicalVolume(SolidAlFrame, //its solid
			fAl, SolidAlFrame->GetName());            //its name
	G4Tubs* SolidHe3 = new G4Tubs("He3Avtive", 0, 4.8 * cm / 2, fBubble_y / 2,
			0., twopi);
	G4LogicalVolume* logicHe3 = new G4LogicalVolume(SolidHe3,        //its solid
...
{
	//Source
	G4double source_rad1 = 0.4 * cm, source_rad2 = 0.25 * cm, source_dz1 = 1
			* cm, source_dz2 = 0.75 * cm;
	G4Tubs* SolidSourceCF = new G4Tubs("CfSource", 0, source_rad2,
			source_dz2 / 2, 0., twopi);
	G4Tubs* SolidSourceSt = new G4Tubs("CfCapsule", 0, source_rad1,
			source_dz1 / 2, 0., twopi);
	G4LogicalVolume* logicSourceCF = new G4LogicalVolume(SolidSourceCF, //its solid
			fCf_src, SolidSourceCF->GetName());            //its name
	G4LogicalVolume* logicSourceSt = new G4LogicalVolume(SolidSourceSt, fSteel,
	...
	G4Box* solidWater = new G4Box("Water", 0.5 * waterX, 0.5 * waterY,
			0.5 * waterZ); //its size
	G4LogicalVolume* logicWater = new G4LogicalVolume(solidWater, fwater,
	...
	visAttOthers->SetForceSolid(true);
	//FiberGlass1
	G4Box* solidFiberGlass1 = new G4Box("FiberGlass1", 0.5 * (102) * cm,
			0.5 * (102) * cm, 0.5 * (101) * cm); //its size
	G4LogicalVolume* logicFiberGlass1 = new G4LogicalVolume(solidFiberGlass1,
			fPyrex, solidFiberGlass1->GetName());            //its name
	...
	G4Box* solidAir1 = new G4Box("Air1", 0.5 * (130) * cm, 0.5 * (130) * cm,
			0.5 * (130) * cm); //its size
	G4LogicalVolume* logicAir1 = new G4LogicalVolume(solidAir1, fAir,
			solidAir1->GetName());            //its name
	G4PhysicalVolumeStore::GetInstance()->Register(new G4PVPlacement(0, //no rotation
	...
	G4Box* solidFiberGlass2 = new G4Box("FiberGlass2", 0.5 * 132 * cm,
			0.5 * 132 * cm, 0.5 * 131 * cm); //its size
	G4LogicalVolume* logicFiberGlass2 = new G4LogicalVolume(solidFiberGlass2,
			fPyrex, solidFiberGlass2->GetName());            //its name
	...
	G4Box* solidAir2 = new G4Box("Air2", 0.5 * 154 * cm, 0.5 * 154 * cm,
			0.5 * 154 * cm); //its size
	G4LogicalVolume* logicAir2 = new G4LogicalVolume(solidAir2, fAir,
	...
	G4Box* solidSteel = new G4Box("Steel", 0.5 * 160 * cm, 0.5 * 160 * cm,
			0.5 * 160 * cm); //its size
	G4LogicalVolume* logicSteel = new G4LogicalVolume(solidSteel, fSteel,
...
void DetectorConstruction::DefineRoom(G4LogicalVolume* logicWorld)
{
	...
	G4Box* solidOutside = new G4Box("OutRoom", 0.5 * frontroomX,
			0.5 * frontroomY, 0.5 * roomY); //its size
	G4LogicalVolume* logicOutside = new G4LogicalVolume(solidOutside, fConcrete,
