...
//----------------------------------------------------------------------------//
void SponFiss_FF::GeneratePrimaryVertex(G4Event* anEvent, G4double time, int mode)
{
	retry:
	if (bfile->eof())
	{
	...
	for (G4int i = nPrompt-1; i > -1; i--)
	{
		...
		G4PrimaryParticle* particle = new G4PrimaryParticle(neutron_definition,
				mom*momx, mom*momy, mom*momz, eng * MeV);
	...
	if (dir.size() > 2)
		for(int n1=0; n1<dir.size(); n1++) {
         double u1 = dir.at(n1).getX(), v1 = dir.at(n1).getY(), w1 = dir.at(n1).getZ();
         for(int n2=n1+1; n2<dir.size(); n2++) 
		 {
            double u2 = dir.at(n2).getX(), v2 = dir.at(n2).getY(), w2 = dir.at(n2).getZ(); 
