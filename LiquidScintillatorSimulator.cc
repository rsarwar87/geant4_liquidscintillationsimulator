...
void PrintUsage() {
  G4cerr << " Usage: " << G4endl;
  G4cerr << " OpNovice [-m macro ] [-u UIsession] [-t nThreads] [-r seed] [-c "
            "cutoff] [-mono beam]"
         << G4endl;
...
int main(int argc, char** argv) {
  ...
  for (G4int i = 1; i < argc; i = i + 2) {
    if (G4String(argv[i]) == "-m")
      ...
      if (atoi(argv[i + 1]) == 1) PrimaryGeneratorAction::beam = true;
...
#else
    UImanager->ApplyCommand("/control/execute LiquidScintillatorSimulator.in");
#endif
